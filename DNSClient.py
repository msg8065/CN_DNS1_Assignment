...
def compare_dns_servers(domainList, question_type):
    for domain_name in domainList:
        ...
        public_answers = query_dns_server(domain_name, question_type)  # Change: Use query_dns_server
        if len(local_answers) != len(public_answers):  # Change: Compare lengths of answers
            return False
...
def local_external_DNS_output(domainList, question_type):
    ...
    for domain_name in domainList:
        answers = query_local_dns_server(domain_name, question_type)  # Change: Use query_local_dns_server
        print(f"The IP address of {domain_name} is {', '.join([answer.to_text() for answer in answers])}")
    print("\nPublic DNS Server")
    for domain_name in domainList:
        answers = query_dns_server(domain_name, question_type)  # Change: Use query_dns_server
        print(f"The IP address of {domain_name} is {', '.join([answer.to_text() for answer in answers])}")
